import 'package:beslen/PagefruitVitamin.dart';
...
class VArasayfa extends StatefulWidget {
  final heroTag;
  final foodName;
  final faydaTag;
  final gramTag;
  ...
  @override
  _VArasayfaState createState() => _VArasayfaState();
}

class _VArasayfaState extends State<VArasayfa> {
  final DocumentReference = Firestore.instance.document("Vitamin");
  List<Widget> _pages;
  Widget _page1;
  Widget _page2;
  Widget _page3;
  ...
  void changeTab(int index) {
    setState(() {
      if (index == 0) {
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => _page1),
        );
      }
      if (index == 1) {
        Navigator.push(
  ...
  initiateSearch(value) {
    ...
    if (queryResultSet.length == 0 && value.length == 1) {
      SearchServiceVitamin().searchByName(value).then((QuerySnapshot docs) {
      ...
      queryResultSet.forEach((element) {
        if (element['vitamin'].startsWith(capitalizedValue)) {
          setState(() {
  ...
  Widget _buildFoodItem(String imgPath, String foodName, String faydaName) {
    return Padding(
        padding:
        ...
        child: InkWell(
            onTap: () {
              Navigator.of(context).push(MaterialPageRoute(
                  builder: (context) => PageFruitVitamin(
                        heroTag: imgPath,
                        foodName: foodName,
                        faydaTag: faydaName,
                      )));
