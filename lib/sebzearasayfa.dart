...
class SArasayfa extends StatefulWidget {
  final heroTag;
  final foodName;
  final faydaTag;
  final gramTag;
  ...
  @override
  _SArasayfaState createState() => _SArasayfaState();
}

class _SArasayfaState extends State<SArasayfa> {
  final DocumentReference = Firestore.instance.document("Sebze");
  List<Widget> _pages;
  Widget _page1;
  Widget _page2;
  Widget _page3;
  ...
  initiateSearch(value) {
    ...
    if (queryResultSet.length == 0 && value.length == 1) {
      SearchServiceSebze().searchByName(value).then((QuerySnapshot docs) {
      ...
      queryResultSet.forEach((element) {
        if (element['sebze'].startsWith(capitalizedValue)) {
          setState(() {
  ...
  Widget _buildFoodItem(String imgPath, String foodName, String faydaName,
      String gramName, String kaloriName, String vitaminName) {
    return Padding(
        padding:
